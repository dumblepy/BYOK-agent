...
import {
  ChatCompletionsRequestMappingError,
  toChatCompletionsRequest,
} from "./openai-chat-completions-request";
import { normalizeChatCompletionsEvents } from "./openai-chat-completions-events";
import { resolveChatCompletionsProfile } from "./openai-chat-completions-profile";
import {
  ChatCompletionsSsePayloadTooLargeError,
  parseChatCompletionsSse,
} from "./openai-chat-completions-sse";
...
export interface OpenAIChatCompletionsAdapterOptions {
  ...
  readonly profile?: Partial<ChatCompletionsProfile>;
}

export class OpenAIChatCompletionsAdapter implements ProviderAdapter {
  public readonly type = "chat-completions";

  private readonly fetchImpl: typeof fetch;
  private readonly timeoutMs: number;
  private readonly maxResponseBytes: number;
  private readonly profile: ChatCompletionsProfile;
  ...
  public constructor(
    ...
    this.profile = resolveChatCompletionsProfile(options.profile ?? init.chatCompletionsProfile);
  }
  ...
  private async *streamInternal(
    ...
    try {
      const body = JSON.stringify(toChatCompletionsRequest(request, this.profile));
      ...
      if (!response.body) {
        yield { type: "error", error: createProviderError("bad-request", request.requestId) };
        return;
      }
      const contentType = response.headers.get("content-type")?.toLowerCase() ?? "";
      if (contentType !== "" && !contentType.includes("text/event-stream")) {
        yield { type: "error", error: createProviderError("unsupported", request.requestId) };
        return;
      }
      const events = parseChatCompletionsSse(
        response.body,
        operation.signal,
        this.maxResponseBytes,
      );
      const toolNames = new Set(request.tools.map((tool) => tool.name));
      for await (const event of normalizeChatCompletionsEvents(
        events,
        request.requestId,
        toolNames,
        operation.signal,
      ...
      yield {
        type: "error",
        error:
          error instanceof ChatCompletionsRequestMappingError ||
          error instanceof ChatCompletionsSsePayloadTooLargeError
...
export class OpenAIChatCompletionsAdapterFactory implements ProviderAdapterFactory {
  public readonly apiType = "chat-completions" as const;

  public constructor(private readonly options: OpenAIChatCompletionsAdapterOptions = {}) {}

  public async create(input: ProviderAdapterInit): Promise<ProviderAdapter> {
    return new OpenAIChatCompletionsAdapter(input, this.options);
  }
}
...
function retryAfterMilliseconds(value: string | null): number | undefined {
  ...
  if (!Number.isSafeInteger(seconds) || seconds > Number.MAX_SAFE_INTEGER / 1000) return undefined;
  return seconds * 1000;
}
